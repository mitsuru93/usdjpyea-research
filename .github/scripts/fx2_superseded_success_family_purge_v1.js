...
function parseIssueBody(body) {
  ...
  const prefixMatch = text.match(/^\s*artifact_prefix:\s*(\S+)\s*$/im);
  const workflowMatch = text.match(/^\s*workflow_name:\s*(.+?)\s*$/im);
  const keepMatch = text.match(/^\s*keep_latest_success:\s*(\d+)\s*$/im);
  if (!prefixMatch) throw new Error('issue body requires artifact_prefix: <prefix>');
  ...
  return { artifactPrefix, workflowName, keepLatestSuccess };
}
...
async function validateFamily({ github, owner, repo, artifacts, artifactPrefix, workflowName, keepLatestSuccess, now }) {
  ...
  if (matching.length > 30) throw new Error(`candidate safety cap exceeded: ${matching.length}`);

  const rows = [];
  for (const artifact of matching) {
    const runId = Number(artifact.workflow_run && artifact.workflow_run.id);
    if (!Number.isInteger(runId) || runId <= 0) throw new Error(`artifact lacks valid run: ${artifact.name}`);
    ...
    if (!['success', 'failure', 'cancelled'].includes(run.conclusion)) {
      throw new Error(`unsupported conclusion for run ${runId}: ${run.conclusion}`);
    }
  ...
  const successes = rows.filter((row) => row.run.conclusion === 'success');
  if (successes.length < keepLatestSuccess) {
    throw new Error(`not enough successful artifacts to preserve: ${successes.length}`);
  }
  const keep = successes.slice(0, keepLatestSuccess);
  const keepIds = new Set(keep.map((row) => Number(row.artifact.id)));
  const newestKeeper = keep[0];

  for (const row of successes.slice(keepLatestSuccess)) {
    ...
    if (!['ahead', 'identical'].includes(comparison.data.status)) {
      throw new Error(
        `newest successful SHA does not supersede run ${row.run.id}: ${comparison.data.status}`
      );
    }
  }
...
async function run({ github, context, core, mode }) {
  ...
  const validation = await validateFamily({
    github,
    owner,
    repo,
    artifacts: before,
  ...
  const selectedBytes = validation.candidates.reduce(
    (sum, row) => sum + Number(row.artifact.size_in_bytes || 0),
    0
  );
  const deleted = [];
  const errors = [];
  ...
  const remaining = after.filter((artifact) => String(artifact.name || '').startsWith(input.artifactPrefix));
  const remainingIds = new Set(remaining.map((artifact) => Number(artifact.id)));
  for (const keeper of validation.keep) {
    if (!remainingIds.has(Number(keeper.artifact.id))) {
      throw new Error(`keeper artifact missing after readback: ${keeper.artifact.id}`);
    }
  }

  const summary = {
    schema_version: 'fx2_superseded_success_family_purge_receipt_v1',
    mode,
    ...
    validated_candidates: validation.candidates.map((row) => ({
      ...
      head_sha: row.run.head_sha
    })),
    generated_at: new Date().toISOString()
  };

  const lines = [
    '## FX2 Superseded-success Family Artifact Purge',
    '',
    `- Mode: \`${mode}\``,
    `- Prefix: \`${input.artifactPrefix}\``,
    `- Exact workflow: \`${input.workflowName}\``,
    `- Successful generations retained: \`${input.keepLatestSuccess}\``,
    ...
    `- Keeper Run IDs: \`${summary.keeper_run_ids.join(', ')}\``,
    `- Errors: \`${summary.error_count}\``,
    '',
    '| Action | Artifact | Run | Conclusion | Size | Head SHA |',
    '|---|---|---:|---|---:|---|'
  ];
  for (const row of validation.keep) {
    lines.push(`| KEEP | \`${row.artifact.name}\` | ${row.run.id} | ${row.run.conclusion} | ${formatBytes(row.artifact.size_in_bytes)} | \`${row.run.head_sha}\` |`);
  }
  for (const row of validation.candidates) {
    lines.push(`| DELETE | \`${row.artifact.name}\` | ${row.run.id} | ${row.run.conclusion} | ${formatBytes(row.artifact.size_in_bytes)} | \`${row.run.head_sha}\` |`);
  }
  ...
  if (mode === 'apply' && remaining.length !== input.keepLatestSuccess) {
    core.setFailed(`expected ${input.keepLatestSuccess} remaining artifacts, found ${remaining.length}`);
  }
  return summary;
}

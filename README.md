...
- Adds `tools/check_research_env.py` for lightweight local preflight checks (imports + config/path sanity).
...
Tool entrypoints under `tools/` are safe to run directly from repo root without manual `PYTHONPATH` setup.

...
A separate development diagnostic will evaluate the forward path of all 13 registered entries without altering the active H2 candidate set.
...
```text
For each of the 13 frozen entry definitions, how does default-cost and severe-cost performance evolve across multiple fixed M15 horizons, and what does the first 24 bars of MFE/MAE show about the entry's price path?
```
...
3. No candidate enters the active H2 because of this analysis.
4. No H2 gate, entry definition, direction, hour window or event exclusion may be changed.
5. The active H2 result remains A1+hold6 and E3+hold6 only.
6. New entry-plus-exit candidates may be proposed only after the full horizon surface is reviewed.
7. Proposed exit families must be small and mechanism-based, rather than a full Cartesian sweep of hold, SL, TP and trailing parameters.
8. Every proposed entry-plus-exit strategy requires a new pre-registration and a later untouched validation block.
9. Research results do not establish MT4 equivalence. Core/MT4 remains the final source of truth.
